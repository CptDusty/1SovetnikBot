...
from db import (get_user_info, check_user, get_users,get_specialities, 
                ...
                get_top_thanks, get_top_thanks_curr, get_top_thanks_prev,
...
async def ty_text(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await kb.edit_keybaoard(callback_query, kb.auto_ty,f"Отлично. Теперь введи текст. Кнопка ^Просто спасибка^ отправит без текста")
...
async def my_ty_date(callback: types.CallbackQuery, state: FSMContext):
    ...
    if table_type == "sent_thanks":
        ...
        text = "Всего отправлено спасибок:"
    ...
    if len(table)>0:
        ...
        for user, messages in user_messages.items():
            ...
            table_text += "\n"
        table_text += "</pre>"
        table_text +=f"ВСЕГО СПАСИБОК: {len(table)}"
...
async def top_table(callback: types.CallbackQuery, state: FSMContext):
    ...
    if period == "all_period":
        table = get_top_thanks()
    elif period == "curr_period":
        table = get_top_thanks_curr()
    elif period == "prev_period":
        table = get_top_thanks_prev()
    if len(table)>0:
        table_text = "<pre>\n"  # Используем тег <pre> для сохранения форматирования
        for row in table:
        ...
        await callback.answer(text= f'Вот топ {period}', reply_markup=ReplyKeyboardRemove())
        await callback.answer(text=table_text, parse_mode='HTML')
        await state.clear()
    else: 
        await callback.answer(text= "Таблица пустая, но в моем алгоритме ты ТОП-1")
        await callback.answer("https://youtube.com/shorts/c4tL6_6lpSA?si=Z79cqwpDoTUoKv7I")
        await state.set_state(Top.period_top)
