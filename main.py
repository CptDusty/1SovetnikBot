...
async def cmd_start(message: Message, state = FSMContext):
    ...
    await bot.send_message(chat_id=message.from_user.id, text='Если была ненужна клавиатура внизу, удалил', reply_markup=ReplyKeyboardRemove())
...
async def menu(message: Message, state = FSMContext):
    if check_user(message.from_user.id):
        ...
        await message.edit_text(text=f"Привет! Я тебя пока не знаю. Незнакомцам  свои функции не показываю {message.from_user.id}", reply_markup=kb.main_new)
